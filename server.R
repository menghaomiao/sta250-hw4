...
shinyServer(function(input, output) {
 output$area=renderPlot({
  ...
  ) #Average signal strength of different routers at each location.
 ...
 output$signal=renderPlot({
  i=as.integer(input$r)
  hx=floor(as.numeric(input$h$x)+0.5) #When mouse hover on 3.5 to 4.5, the actual point is 4.
  hy=floor(as.numeric(input$h$y)+0.5)
  d=with(sub[[i]], circular(orientation[x==hx & y==hy]+90, units='degrees')) #Ajust orientation.
  s=with(sub[[i]], -signal[x==hx & y==hy]-24) #Transform signal scale so that it's from 1 to 69.
  if (length(d)!=0) {
   windrose(d, s, breaks, main='', fill=colors)
  } else {
   windrose(breaks, rep(1, 8), breaks, main='', fill=F) #If NULL, draw a blank plot.
  }
 })
})
